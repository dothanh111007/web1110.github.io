var pJS = function (e, i) {
    ...
    t.fn.canvasSize = function () {
        ...
        if (t && t.interactivity.events.resize) 
    ...
    t.fn.canvasPaint = function () {
        t
            .canvas
            .ctx
            .fillRect(0, 0, t.canvas.w, t.canvas.h)
    };
    t.fn.canvasClear = function () {
        t
            .canvas
            .ctx
            .clearRect(0, 0, t.canvas.w, t.canvas.h)
    };
    t.fn.particle = function (e, i, a) {
        this.radius = (
            t.particles.size.random
                ? Math.random()
                : 1
        ...
        if (t.particles.size.anim.enable) {
            ...
            if (!t.particles.size.anim.sync) 
                this.vs = this.vs * Math.random()
        }
        this.x = a
            ? a.x
            : Math.random() * t.canvas.w;
        this.y = a
            ? a.y
            : Math.random() * t.canvas.h;
        if (this.x > t.canvas.w - 2 * this.radius) 
            this.x = this.x - this.radius;
        else if (this.x < 2 * this.radius) 
            this.x = this.x + this.radius;
        if (this.y > t.canvas.h - 2 * this.radius) 
            this.y = this.y - this.radius;
        else if (this.y < 2 * this.radius) 
            this.y = this.y + this.radius;
        if (t.particles.move.bounce) 
            t
                .fn
                .vendors
                .checkOverlap(this, a);
        ...
        if (t.particles.opacity.anim.enable) {
            ...
            if (!t.particles.opacity.anim.sync) 
    ...
    t.fn.particle.prototype.draw = function () {
        ...
        if (void 0 != e.opacity_bubble) 
            ...
            var a = e.opacity;
        if (e.color.rgb) 
            var s = "rgba(" + e.color.rgb.r + "," + e.color.rgb.g + "," + e.color.rgb.b +
                    "," + a + ")";
        else 
            var s = "hsla(" + e.color.hsl.h + "," + e.color.hsl.s + "%," + e.color.hsl.l +
                    "%," + a + ")";
        ...
        switch (e.shape) {
            case "circle":
                t
                    .canvas
                    .ctx
                    .arc(e.x, e.y, i, 0, 2 * Math.PI, false);
                break;
            case "edge":
                t
                    .canvas
                    .ctx
                    .rect(e.x - i, e.y - i, 2 * i, 2 * i);
                break;
            case "triangle":
                t
                    .fn
                    .vendors
                    .drawShape(t.canvas.ctx, e.x - i, e.y + i / 1.66, 2 * i, 3, 2);
                break;
            case "polygon":
                t
                    .fn
                    ...
                    .drawShape(
                        t.canvas.ctx,
                        e.x - i / (t.particles.shape.polygon.nb_sides / 3.5),
                        e.y - i / (2.66 / 3.5),
                        2.66 * i / (t.particles.shape.polygon.nb_sides / 3),
            ...
            case "star":
                t
                    ...
                    .drawShape(
                        t.canvas.ctx,
                        e.x - 2 * i / (t.particles.shape.polygon.nb_sides / 4),
                        e.y - i / (2 * 2.66 / 3.5),
                        2 * i * 2.66 / (t.particles.shape.polygon.nb_sides / 3),
                        t.particles.shape.polygon.nb_sides,
                        2
                    );
                break;
            case "image":
                function r() {
                    t
                        .canvas
                        .ctx
                        .drawImage(n, e.x - i, e.y - i, 2 * i, 2 * i / e.img.ratio)
                }
    ...
    t.fn.particlesCreate = function () {
        for (var e = 0; e < t.particles.number.value; e++) 
            t
                .particles
                .array
                .push(new t.fn.particle(t.particles.color, t.particles.opacity.value))
        };
        ...
        for (var e = 0; e < t.particles.array.length; e++) {
            ...
            if (t.particles.opacity.anim.enable) {
                if (true == i.opacity_status) {
                    if (i.opacity >= t.particles.opacity.value) 
                        i.opacity_status = false;
                    i.opacity += i.vo
                } else {
                    if (i.opacity <= t.particles.opacity.anim.opacity_min) 
                        i.opacity_status = true;
                    i.opacity -= i.vo
                }
                if (i.opacity < 0) 
                    i.opacity = 0
            ...
            if (t.particles.size.anim.enable) {
                if (true == i.size_status) {
                    if (i.radius >= t.particles.size.value) 
                        i.size_status = false;
                    i.radius += i.vs
                } else {
                    if (i.radius <= t.particles.size.anim.size_min) 
                        i.size_status = true;
                    i.radius -= i.vs
                }
                if (i.radius < 0) 
            ...
            if (isInArray("grab", t.interactivity.events.onhover.mode)) 
                t
                    .fn
                    .modes
                    .grabParticle(i);
            if (isInArray("bubble", t.interactivity.events.onhover.mode) || isInArray("bubble", t.interactivity.events.onclick.mode)) 
                t
                    .fn
                    .modes
                    .bubbleParticle(i);
            if (isInArray("repulse", t.interactivity.events.onhover.mode) || isInArray("repulse", t.interactivity.events.onclick.mode)) 
                t
                    .fn
                    .modes
                    .repulseParticle(i);
            if (t.particles.line_linked.enable || t.particles.move.attract.enable) 
                for (var r = e + 1; r < t.particles.array.length; r++) {
                    var n = t
                        .particles
                        .array[r];
                    if (t.particles.line_linked.enable) 
                        t
                            .fn
                            .interact
                            .linkParticles(i, n);
                    if (t.particles.move.attract.enable) 
                        t
                            .fn
                            .interact
                            .attractParticles(i, n);
                    if (t.particles.move.bounce) 
                        t
                            .fn
                            .interact
    ...
    t.fn.interact.linkParticles = function (e, i) {
        ...
        if (r <= t.particles.line_linked.distance) {
            var n = t.particles.line_linked.opacity - r / (
    ...
    t.fn.modes.bubbleParticle = function (e) {
        if (t.interactivity.events.onhover.enable && isInArray("bubble", t.interactivity.events.onhover.mode)) {
            ...
            if (s <= t.interactivity.modes.bubble.distance) {
                if (r >= 0 && "mousemove" == t.interactivity.status) {
                    ...
                    if (t.interactivity.modes.bubble.opacity != t.particles.opacity.value) 
                        if (t.interactivity.modes.bubble.opacity > t.particles.opacity.value) {
                            ...
                            if (l > e.opacity && l <= t.interactivity.modes.bubble.opacity) 
                                e.opacity_bubble = l
                        }
                    else {
                        var l = e.opacity - (
                            t.particles.opacity.value - t.interactivity.modes.bubble.opacity
                        ) * r;
                        if (l < e.opacity && l >= t.interactivity.modes.bubble.opacity) 
                            e.opacity_bubble = l
                    }
                ...
                n();
            if ("mouseleave" == t.interactivity.status) 
                n()
            ...
            if (t.tmp.bubble_clicking) {
                ...
                if (v > t.interactivity.modes.bubble.duration) 
                    t.tmp.bubble_duration_end = true;
            ...
            if (t.tmp.bubble_clicking) {
                ...
                p(
                    t.interactivity.modes.bubble.opacity,
                    t.particles.opacity.value,
                    e.opacity_bubble,
                    e.opacity,
                    "opacity"
    ...
    t.fn.modes.repulseParticle = function (e) {
        if (t.interactivity.events.onhover.enable && isInArray("repulse", t.interactivity.events.onhover.mode) && "mousemove" == t.interactivity.status) {
            ...
            if ("bounce" == t.particles.move.out_mode) {
                if (l.x - e.radius > 0 && l.x + e.radius < t.canvas.w) 
                    e.x = l.x;
                if (l.y - e.radius > 0 && l.y + e.radius < t.canvas.h) 
                    e.y = l.y
            } else {
                e.x = l.x;
                e.y = l.y
            ...
            if (!t.tmp.repulse_finish) {
                t.tmp.repulse_count++;
                if (t.tmp.repulse_count == t.particles.array.length) 
                    t.tmp.repulse_finish = true
            }
            if (t.tmp.repulse_clicking) {
                ...
                function u() {
                    ...
                    if ("bounce" == t.particles.move.out_mode) {
                        ...
                        if (a.x + e.radius > t.canvas.w) 
                            e.vx = -e.vx;
                        else if (a.x - e.radius < 0) 
                            e.vx = -e.vx;
                        if (a.y + e.radius > t.canvas.h) 
                            e.vy = -e.vy;
                        else if (a.y - e.radius < 0) 
                            e.vy = -e.vy
                    }
                }
                if (d <= n) 
    ...
    t.fn.modes.grabParticle = function (e) {
        if (t.interactivity.events.onhover.enable && "mousemove" == t.interactivity.status) {
            ...
            if (s <= t.interactivity.modes.grab.distance) {
                var r = t.interactivity.modes.grab.line_linked.opacity - s / (
                ...
                if (r > 0) {
                    ...
                    t
                        .canvas
                        .ctx
                        .lineTo(t.interactivity.mouse.pos_x, t.interactivity.mouse.pos_y);
                    t
                        .canvas
                        .ctx
                        .stroke();
    ...
    t.fn.vendors.eventsListeners = function () {
        if ("window" == t.interactivity.detect_on) 
        ...
        if (t.interactivity.events.onclick.enable) 
            t
                .interactivity
                .el
                            ...
                            case "push":
                                if (t.particles.move.enable) 
                                    t
                                        .fn
                                        .modes
                                        .pushParticles(t.interactivity.modes.push.particles_nb, t.interactivity.mouse);
                                else if (1 == t.interactivity.modes.push.particles_nb) 
                                    t
                                        .fn
                                        .modes
                                        .pushParticles(t.interactivity.modes.push.particles_nb, t.interactivity.mouse);
                                else if (t.interactivity.modes.push.particles_nb > 1) 
                                    t
                                        .fn
                                        .modes
                                        .pushParticles(t.interactivity.modes.push.particles_nb);
                                break;
                            case "remove":
                                t
                                    .fn
                                    .modes
                                    .removeParticles(t.interactivity.modes.remove.particles_nb);
                                break;
                            case "bubble":
    ...
    t.fn.vendors.createSvgImg = function (e) {
        var i = t.tmp.source_svg,
            ...
            s = i.replace(a, function (i, a, t, s) {
                if (e.color.rgb) 
                    var r = "rgba(" + e.color.rgb.r + "," + e.color.rgb.g + "," + e.color.rgb.b +
                            "," + e.opacity + ")";
                else 
                    var r = "hsla(" + e.color.hsl.h + "," + e.color.hsl.s + "%," + e.color.hsl.l +
                            "%," + e.opacity + ")";
                return r
            });
        var r = new Blob([s], {type: "image/svg+xml;charset=utf-8"}),
    ...
    t.fn.vendors.checkBeforeDraw = function () {
        if ("image" == t.particles.shape.type) 
            if ("svg" == t.tmp.img_type && void 0 == t.tmp.source_svg) 
                t.tmp.checkAnimFrame = requestAnimFrame(check);
            else {
    ...
    t.fn.vendors.start = function () {
        if (isInArray("image", t.particles.shape.type)) {
            t.tmp.img_type = t
                ...
                .image
                .src
                .substr(t.particles.shape.image.src.length - 3);
            t
                .fn
                .vendors
                .loadImg(t.tmp.img_type)
        } else 
            t
                .fn
                .vendors
...
// phát hiện xem ứng dụng có đang chạy ở chế độ độc lập hay không
if (window.matchMedia('(display-mode: standalone)').matches) {
    // thêm lớp hidden vào phần tử popup
    document.getElementById('popup').classList.add('hidden');
  } else {
    // xóa lớp hidden khỏi phần tử popup
  ...
  if (window.matchMedia('(display-mode: standalone)').matches) {
    // Trình duyệt đang chạy trong chế độ PWA, ẩn các phần tử không cần thiết
    document.getElementById('popup').style.display = 'none';
  } else {
    // Trình duyệt đang chạy trong chế độ truy cập web, hiển thị các phần tử
  ...
  if (isIOS) {
    // Nếu đang truy cập trang web trên iOS
    //document.getElementById("popup").style.display = "block";
  } else {
    // Nếu đang truy cập trang web trên các thiết bị khác
    document.getElementById("popup").style.display = "none";
  }
